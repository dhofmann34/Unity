...
class Trainer():
    ...
    def coteaching_plus_sgd(self, dataloader, y_noisy, threshold_outlier, threshold_inlier, outlier_ratio):
        '''
        ...
        for iter_num, (idx, data, label) in enumerate(dataloader):
            ...
            # select small loss samples that are in disagreement as clean
            disagree_idx = torch.flatten(torch.nonzero(model1_pred_label != model2_pred_label))  # disagree indexes

            if self.args.strategy == "default":
                num_sample = int(len(idx)*threshold_outlier)  # number of samples to select as clean
                model1_select_idx = torch.argsort(loss_model1[disagree_idx], descending=False)[:num_sample]
                model2_select_idx = torch.argsort(loss_model2[disagree_idx], descending=False)[:num_sample]

                model1_select_idx = disagree_idx[model1_select_idx]
                model2_select_idx = disagree_idx[model2_select_idx]

            elif self.args.strategy == "split":
                # split into pred inliers and outliers
                model1_inlier_idx = torch.nonzero(model1_pred_label[disagree_idx] == 0).flatten().cpu().cuda()
                model1_outlier_idx = torch.nonzero(model1_pred_label[disagree_idx] == 1).flatten().cpu().cuda()
                model2_inlier_idx = torch.nonzero(model2_pred_label[disagree_idx] == 0).flatten().cpu().cuda()
                model2_outlier_idx = torch.nonzero(model2_pred_label[disagree_idx] == 1).flatten().cpu().cuda()
                ...
                loss_model1_inlier = loss_model1[disagree_idx][model1_inlier_idx]
                loss_model1_outlier = loss_model1[disagree_idx][model1_outlier_idx]
                loss_model2_inlier = loss_model2[disagree_idx][model2_inlier_idx]
                loss_model2_outlier = loss_model2[disagree_idx][model2_outlier_idx]
                ...
                model1_select_idx = torch.cat((disagree_idx[model1_inlier_idx[selected_model1_inlier]], disagree_idx[model1_outlier_idx[selected_model1_outlier]]))
                model2_select_idx = torch.cat((disagree_idx[model2_inlier_idx[selected_model2_inlier]], disagree_idx[model2_outlier_idx[selected_model2_outlier]]))
    ...
    def train_coteaching_plus(self, dataloader, y_noisy, outlier_ratio, noise_rate_outlier, noise_rate_inlier):
        '''
        ...
        for epoch in range(self.args.epochs):
            ...
            # SGD
            self.coteaching_plus_sgd(dataloader, y_noisy, threshold_outlier, threshold_inlier, outlier_ratio)
...
def run_coteaching_plus(args, dataloader, dim, y_noisy, outlier_ratio, noise_rate_outlier, noise_rate_inlier):
    '''
    ...
    trainer.train_coteaching_plus(dataloader, (torch.eye(2, device='cuda:0')[y_noisy]), outlier_ratio, noise_rate_outlier, noise_rate_inlier)  # train models