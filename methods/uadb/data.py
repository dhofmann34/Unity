...
class TabularData(object):
    def __init__(self, config, X, y, outlier_score) -> None:
        self.config = config
        #self.inputs, self.outputs = load_data(self.config)

        self.inputs = X  # use our input data
        self.outputs = y  # use our GT labels
        self.input_dim = self.inputs.shape[1]
        #self.pseudo_labels = self.get_init_labels()  # we want to use our noisy labels

        self.pseudo_labels = np.reshape(outlier_score, (len(outlier_score), 1))
    ...
    def build_train_dataloaders(self):
        ...
        train_inputs = torch.from_numpy(self.inputs).float()
        train_outputs = torch.from_numpy(self.outputs).float()
        train_pseudo_labels = torch.from_numpy(self.pseudo_labels).float()

        self.train_loaders.append(
            self._build_dataloader(TensorDataset(train_inputs, train_outputs, train_pseudo_labels), shuffle=True)
        )

        val_inputs = torch.from_numpy(self.inputs).float()
        val_outputs = torch.from_numpy(self.outputs).float()
        val_pseudo_labels = torch.from_numpy(self.pseudo_labels).float()
